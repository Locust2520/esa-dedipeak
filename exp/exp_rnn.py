...
class Exp_RNN(Exp_Basic):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, mean, scale) in tqdm(enumerate(vali_loader)):
                ...
                if self.args.loss == 'evl':
                    batch_all = torch.cat([batch_x, batch_y], dim=1).detach()
                    vs = self.model.extreme_values(batch_all)
                    vs = vs[:, -self.args.pred_len:, :]

                outputs = []
                for j in range(self.args.pred_len):
                    if self.args.loss == 'evl':
                        output, us = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                        loss = self.model.l1(output, batch_y[:, j:j+1], us, vs[:, j:j+1])
                        total_loss.append(loss.item())
                    else:
                        output = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                    batch_x = torch.cat([batch_x, output], dim=1).detach()
    ...
    def train(self, setting):
        ...
        elif self.args.loss == 'evl':
            criterion_tmp = self.model.l1
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, mean, scale) in pbar:
                ...
                for j in range(self.args.pred_len):
                    if self.args.loss == 'evl':
                        output, us = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                        loss = criterion_tmp(output, batch_y[:, j:j+1], us, vs[:, j:j+1])
                    else:
                        output = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                        if self.args.loss == 'adaptive':
                            loss = criterion_tmp((output-batch_y[:, j:j+1]).flatten().unsqueeze(-1))
                        else:
                            loss = criterion_tmp(output, batch_y[:, j:j+1])
                    batch_x = torch.cat([batch_x, batch_y[:, j:j+1]], dim=1).detach()
                    outputs.append(output)
                    loss = loss.mean()
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, mean, scale) in tqdm(enumerate(test_loader)):
                ...
                if i % save_every == 0:
                    ...
                    if self.args.use_multi_scale:
                        for scale, output in zip(self.args.scales, outputs):
